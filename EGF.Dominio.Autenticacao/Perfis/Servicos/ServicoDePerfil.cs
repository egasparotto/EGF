...
namespace EGF.Dominio.Autenticacao.Perfis.Servicos
{
    public class ServicoDePerfil : ServicoDePersistencia<Perfil, IRepositorioDePerfil>, IServicoDePerfil
    {
        ...
        public async Task<string> GetNormalizedRoleNameAsync(Perfil role, CancellationToken cancellationToken)
        {
            var retorno = await Repositorio.BuscarAsync(x => x.Id == role.Id);
            return retorno.FirstOrDefault()?.CodigoInterno?.ToUpper();
        }

        public async Task<string> GetRoleIdAsync(Perfil role, CancellationToken cancellationToken)
        {
            var retorno = await Repositorio.BuscarAsync(x => x.Id == role.Id);
            return retorno.FirstOrDefault()?.Id.ToString();
        }

        public async Task<string> GetRoleNameAsync(Perfil role, CancellationToken cancellationToken)
        {
            var retorno = await Repositorio.BuscarAsync(x => x.Id == role.Id);
